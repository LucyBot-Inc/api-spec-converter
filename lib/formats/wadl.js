...
function convertToSwagger(wadl) {
  ...
  function convertMethod(wadlMethod) {
    ...
    var wadlResponse = unwrapArray(wadlMethod.response);
    if (wadlResponse && wadlResponse.representation && wadlResponse.representation.length) {
      wadlResponse.representation.forEach (function (representation) {
        if (typeof representation.$ && representation.$.mediaType) {
          if (!method.produces) {
            method.produces = [];
          }
          method.produces.push(representation.$.mediaType);
        }
      });
    }
