...
function convertOperations(openApiSpec) {
    var path, pathObject, method, operation, params;
    for (path in openApiSpec.paths) {
        ...
        for (method in pathObject) {
            if (HTTP_METHODS.indexOf(method) >= 0) {
                ...
                //if there is a top level path param available it will use if there is no method param available already 
                if(pathObject.parameters){
                    if(!operation.parameters) operation.parameters = pathObject.parameters;
                    else operation.parameters.push(pathObject.parameters)
                };

                convertParameters(openApiSpec, operation);
                convertResponses(openApiSpec, operation);
                if(!operation.operationId) operation.operationId = path.replace(/[^a-z\/A-Z]/g,'').replace(/\/\//g,'').replace(/\//g, "_")+'_'+method;
            }
        }
        //NOTE: remove parameters from the output, if this shows in the swagger-generate at the root of the path it will fail
        ...
        if (content) {
            ...
            if (content['application/x-www-form-urlencoded']) {
                ...
                if (param.schema.type === 'object' && param.schema.properties) {
                    ...
                    for (var name in param.schema.required){
                        var req = param.schema.required[name];
                        if(param.schema.properties[req]) param.schema.properties[req]['required'] = true; 
                    }
                } else {
