...
Swagger1.prototype.maybeResolveFromUrl = function(callback) {
  var self = this;
  if (!self.url) return callback();
  Request(self.url, function(err, response, spec) {
    if (err) throw err;
    self.spec = Utils.parse(spec);
    var baseUrl = self.spec.basePath;
    if (!baseUrl || !containsHost(baseUrl)) {
      baseUrl = Url.parse(self.url);
      if (self.spec.basePath) baseUrl.pathname = self.spec.basePath;
    } else {
      baseUrl = Url.parse(baseUrl);
    }
    if (!baseUrl.query) {
      baseUrl.query = Url.parse(self.url).query;
    }
