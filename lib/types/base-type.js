...
BaseType.prototype.resolveSubResources = function (source, sourceType, callback) {
  callback(null);
};
...
BaseType.prototype.resolveResources = function(source, callback) {
  ...
  self.readSpec(source, function(err, spec, sourceType) {
    if (err) return callback(err);
    ...
    //In make cases URL has some usefull info
    if (sourceType === 'url')
      self.url = source;

    self.resolveSubResources(source, sourceType, function (error) {
      if (error) return callback(error);

      self.fixSpec();

      var version = self.getFormatVersion();
